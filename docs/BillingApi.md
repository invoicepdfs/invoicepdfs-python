...
with invoicepdfs.ApiClient(configuration) as api_client:
    # Create an instance of the API class
    api_instance = invoicepdfs.BillingApi(api_client)
    billing_overage_request = invoicepdfs.BillingOverageRequest() # BillingOverageRequest | 

    try:
        # Update Overage Settings
        api_response = api_instance.update_overage_settings(billing_overage_request)
        print("The response of BillingApi->update_overage_settings:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling BillingApi->update_overage_settings: %s\n" % e)
```
